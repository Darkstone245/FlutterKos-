...
List<int> prepare_Ant_Message(int Page, controllerState State) {
  State.Travel_Mode_State = State.Regen_Level | State.Assist_Level << 3;
  List<int> message = List<int>.filled(12, 0);
...
controllerState processRxAnt(List<int> RxAnt, controllerState State) {
  int chkSum = 0;
  int page = RxAnt[3];
  for (var i = 0; i < 11; i++) {
    chkSum ^= RxAnt[i];
...
controllerState assignJSON_CS(Map<String, dynamic> mapJSON, controllerState State) {
  State.throttleMax = mapJSON['throttleMax'] as int? ?? State.throttleMax;
  State.throttleOffset = mapJSON['throttleOffset'] as int? ?? State.throttleOffset;
  State.Assist_Level = mapJSON['assistLevel'] as int? ?? State.Assist_Level;
  State.Regen_Level = mapJSON['regenLevel'] as int? ?? State.Regen_Level;
